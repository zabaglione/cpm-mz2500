...
- 58Kシステム（TPA 52.25KB）、80×25テキスト。画面制御はADM-3A互換に加えて
  ANSI/VT100サブセット（`ESC[` のカーソル位置指定・消去・反転表示など）と
...
CP/M版Rogue 1.7（1985年）にはライセンス表記がないため、このリポジトリや
リリースには**同梱していません**。かわりに、ご自分のマシン上でディスクを
組み立てるスクリプトを用意しています（取得は全てSHA256検証付き。生成した
ディスクはご自身の私的利用の範囲でお使いください）。

必要なのは **Python 3 だけ**です:

```
git clone https://github.com/zabaglione/cpm-mz2500.git
cd cpm-mz2500
python3 tools/make_rogue_disk.py
```

これで `build/rogue.d88`（起動可能フロッピー）が完成します。CP/M本体は
ローカルでビルド済みならそれを、なければリリース版を自動で取得して
使います（z80asmは不要）。
...
1. **ブラウザで** —
   ...
   を開き、`rogue.d88` を画面にドラッグ&ドロップ。CP/Mが起動したら
   `A>` で `ROGUE` と入力
2. **実機で** — 「実機で — フロッピー」の手順どおり `rogue.d88` を
   フロッピーに書いて起動し、`A>` で `ROGUE`

移動は h j k l（斜めは y u b n）、コマンド一覧は `TYPE ROGUE.DOC` で
読めます。セーブ（S コマンド）は同じディスクに書かれます。ブラウザ版なら
ディスクへの書き込みはブラウザ内に保存され、次回も続きから遊べます。
