...
リリースには**同梱していません**。かわりに、ご自分のマシンで取りに行って
ディスクを組み立てるスクリプトを用意しています（取得はSHA256検証付き。
生成したディスクはご自身の私的利用の範囲でお使いください）:

```
python3 tools/fetch_cpm22.py               # 初回のみ
make disks                                 # ブートFDを生成
python3 tools/make_rogue_disk.py           # build/rogue.d88 が完成
```

`rogue.d88` は起動可能フロッピーです。
[ブラウザ版エミュレータ](https://zabaglione.github.io/mz2500-web-emulator/)
の画面にドラッグ&ドロップすると CP/M が起動するので、`A>` で `ROGUE` と
入力してください。実機ではフロッピーに書き込んで起動するだけです。
移動は h j k l（斜めは y u b n）、遊び方は `TYPE ROGUE.DOC` で読めます。
セーブ（S コマンド）は同じディスクに書かれます。
