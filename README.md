...
**v3.1.2**、使用するDRI純正BDOSの番号は **3.1** です。表示だけを変更した
...
**v3.1.2では新規ディスクイメージを使用してください。** バンク版BDOSの起動領域を
...
- 現行Plus版でのゲーム別確認結果とビルド手順は[ゲームカタログ](GAMES.md)を参照してください（CatChumのみ2.2版限定）。
...
起動・初回ドライブ選択のバンク転送を最適化しています。[速度比較と検証](docs/performance.md)を参照してください。
